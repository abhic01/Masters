...
from draft import ALL_SLOTS, BACKUP_SLOTS, DraftConfig, DraftState, Pick, SLOT_LABELS, STARTER_SLOTS
...
def seed_room_from_saved_state():
    ...
    d.picks = []
    for pick in draft_state.get("picks", []) or []:
        if isinstance(pick, dict):
            d.picks.append(Pick(
                pick_no=int(pick.get("pickNo", 0) or 0),
                team=pick.get("team", ""),
                athlete_id=pick.get("athleteId", ""),
                name=pick.get("name", ""),
                slot=pick.get("slot", ""),
                slot_label=pick.get("slotLabel", SLOT_LABELS.get(pick.get("slot", ""), pick.get("slot", ""))),
                ts=float(pick.get("ts", 0) or 0),
            ))
